...
function getVersionInfo() {
    const scriptTags = document.querySelectorAll('script[src*="jsdelivr.net"]');
    let repoInfo = 'RC-Race-Results-Generator (Local)';
    
    scriptTags.forEach(script => {
        const src = script.src;
        const match = src.match(/jsdelivr\.net\/gh\/([^\/]+)\/([^\/]+)@([^\/]+)/);
        if (match) {
            const [, owner, repo, version] = match;
            repoInfo = `${owner}/${repo}@${version}`;
        }
    });
...
function generateImage() {
    ...
    tempContainer.style.fontFamily = "'メイリオ', 'Meiryo', sans-serif";
    ...
    versionDiv.style.borderTop = '1px solid #ddd';
    versionDiv.style.paddingTop = '8px';
    versionDiv.innerHTML = `Generated by: ${versionInfo}<br>Date: ${currentDate}`;
    
    tempContainer.appendChild(versionDiv);
...
// DOM読み込み完了時にバージョン情報を表示
document.addEventListener('DOMContentLoaded', function() {
    const versionInfoElement = document.getElementById('repoInfo');
    if (versionInfoElement) {
        versionInfoElement.textContent = getVersionInfo();
    }
});
